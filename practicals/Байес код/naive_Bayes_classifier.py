...
train_data=[]
with open("data_for_bayes.txt", encoding = 'utf-8') as file:
    #file = open("data_for_bayes.txt", encoding = 'utf-8')
...
print("Категориальные признаки: {0} \nЦелевая переменная: {1} \nКоличество классов: {2}".format(number_of_categorical_attribute,target_var,number_of_classes))
...
# %%
# def get_probability_class(str):
#     count = 0
#     for i in range(number_of_data):
#         if(data[i,target_var] == str):
#             count+=1
#     return count
# apriori_class_probability = list(map(get_probability_class, uniq_elements[target_var]))
# print(apriori_class_probability)

count_el_of_class=[]
#Число объектов в разных классах
for c in uniq_elements[target_var][1]:
    count_el_of_class.append(list(data[:,target_var]).count(c))
print(count_el_of_class)
...
for ind_cl,cl in enumerate(uniq_elements[target_var][1]):#Для каждого класса
    cond_prob_for_class = []
    for att_ind in number_of_categorical_attribute:  #Для категориальных аттрибутов
...
def get_dispersion_and_math_expected(column,cl):#Для класса
    list_result = []
